...
export function ImportPage() {
    ...
    const createAutomaticMappings = (preview: CSVPreview) => {
        ...
        // Apply known mappings if columns exist
        preview.headers.forEach(header => {
            const lowerHeader = header.toLowerCase();
            
            // Exact match
            if (knownMappings[header]) {
                autoMappings[knownMappings[header]] = header;
            }
            // Case-insensitive partial match
            else {
                // For content field (most important)
                if (lowerHeader.includes('text') || lowerHeader.includes('content')) {
                    autoMappings['content'] = header;
                }
                // Other common fields
                else if (lowerHeader.includes('reply') || lowerHeader.includes('response')) {
                    autoMappings['reply_to'] = header;
                }
                else if (lowerHeader.includes('id') && lowerHeader.includes('turn')) {
                    autoMappings['turn_id'] = header;
                }
                else if (lowerHeader.includes('user') || lowerHeader.includes('author')) {
                    autoMappings['author'] = header;
                }
                else if (lowerHeader.includes('thread')) {
                    autoMappings['thread'] = header;
                }
                else if (lowerHeader.includes('time') || lowerHeader.includes('date')) {
                    autoMappings['timestamp'] = header;
                }
            }
        });
        
        // Ensure content is mapped - critical field
        if (!autoMappings['content'] && preview.headers.includes('turn_text')) {
            autoMappings['content'] = 'turn_text';
        }
        
        console.log('Automatic mappings applied:', autoMappings);
